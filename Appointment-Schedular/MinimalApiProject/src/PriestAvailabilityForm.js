        ...
        <>
          {profile && profile.role === 'Admin' && (
            <div className="admin-panel">
              ...
              <button onClick={() => resetForm()}>Add Availability</button>
            </div>
          )}
          ...
          <div className="calendar-grid">
            {Array.from({ length: daysInMonth(selectedMonth, selectedYear) }, (_, day) => (
              <div
                key={day + 1}
                className={`calendar-day ${selectedDay === day + 1 ? 'selected' : ''} ${isAvailableDay(day + 1) ? 'available' : ''}`}
                ...
                {availabilities
                  ...
                  .map((availability) => (
                    <li key={availability.id}>
                      ...
                      <button onClick={() => handleEditClick(availability)}>Edit Availability</button>
                    </li>
                  ))}
              </ul>
            </div>
          ...
          {editingAvailability && (
            <div className="edit-availability">
              ...
              <form onSubmit={(e) => { e.preventDefault(); handleUpdateAvailability(); }}>
                <label>Start Date</label>
                <input type="date" name="startDate" value={newAvailability.startDate} onChange={handleInputChange} />
                <label>End Date</label>
                <input type="date" name="endDate" value={newAvailability.endDate} onChange={handleInputChange} />
                <label>Days</label>
                  ...
                  {['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'].map((day) => (
                    <div key={day}>
                      <input
                        ...
                        name="days"
                        value={day}
                        checked={newAvailability.days.includes(day)}
